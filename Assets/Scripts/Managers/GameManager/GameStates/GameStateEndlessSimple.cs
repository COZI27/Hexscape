...
{
    private const string Path = "Levels/EndlessSimple";
    ...
    public float playerSpeedIncreaseLogMultiplyer = 25f;
    ...
    float maxClickEnergy = 1f;
    float clickEnergyDrain = 1f;
    float clickEnergyRechargePerSecondBase = 2.4f;
    float clickEnergyRechargePerSecondPlayerSpeedMod = 0.005f;
    ...
    public override void StateUpdate()
    {
        ...
        energyMetreTunnel.SetEnergyFill((float)currentClickEnergy / (float)maxClickEnergy, (float)currentClickEnergy / (float)maxClickEnergy, false);
       ...
       currentClickEnergy+= GetClickEnergyRecharge() * Time.deltaTime;
        currentClickEnergy = Mathf.Clamp(currentClickEnergy, 0, maxClickEnergy);

       // Debug.Log(MapSpawner.Instance.GetCurrentMapHolder().transform.position.y);

      

        if (!hasThud || currentClickEnergy < clickEnergyDrain || GameManager.instance.GetPlayerBall().transform.position.y < MapSpawner.Instance.GetCurrentMapHolder().transform.position.y)
        {
            SetMapActive(false);
        } else
        {
    ...
    private void MaxEnergyReachedListener()
    {
        ...
        ColourManager.instance.ChangePalette(currentSessionData.levelIndex);
    }
    ...
    public override void StartGameState()
    {
        ...
        CameraFollow.GetInstance().snapToMapCentre = true;
    ...
    public override void HexDigEvent(Hex hex)
    {
        ...
        currentSessionData.levelScore += points;
        currentSessionData.totalScore += points;
        ...
        {
            if (playerController.transform.position.y >= MapSpawner.Instance.GetCurrentGrid().transform.position.y)
            {
    ...
    private void PopulateLevelsArray()
    {
        ...
        levels = LevelLoader.Instance.GetLevelsFrom(Path);
    }
