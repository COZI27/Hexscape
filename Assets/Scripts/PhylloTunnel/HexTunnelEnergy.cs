...
public class HexTunnelEnergy : ObserverPattern.Observer
{
    ...
    {
        ColourManager.instance.RemoveObserver(this);
    ...
    public void SetEnergyFill(float energyFill, float accelerationFill = 0, bool lerpFill = true)
    {
        ...
        //Debug.Log("SetEnergyFill(float energyFill, float accelerationFill = 0, bool lerpFill = true) " + energyFill);
       // Debug.Log("TargetFill = " + targetEnergyFill);
    ...
    public override void OnNotify()
    {
        if (true /*test whether colour change is appropriate*/)
        {
            ...
            Gradient g = ColourManager.instance.GetGradientFromPalette();
            //energyAccelerationFillMat.SetColor("_Colour", g.colorKeys[0].color);
            //energyFillMat.SetColor("_Colour", g.colorKeys[1].color);

            StartCoroutine(LerpToColour(g));
        }
    }

    private IEnumerator LerpToColour(Gradient gradient)
    {
        Color startColourA = energyAccelerationFillMat.GetColor("_Colour");
        Color targetColourA = gradient.colorKeys[1].color;
        Color colourToLerpA;

        Color startColourE = energyFillMat.GetColor("_Colour");
        Color targetColourE = gradient.colorKeys[2].color;
        Color colourToLerpE;
        ...
        while (elapsedTime < totalTime)
        {
            ...
            colourToLerpA = Color.Lerp(startColourA, targetColourA, (elapsedTime / totalTime));
            energyAccelerationFillMat.SetColor("_Colour", colourToLerpA);

            colourToLerpE = Color.Lerp(startColourE, targetColourE, (elapsedTime / totalTime));
            energyFillMat.SetColor("_Colour", colourToLerpE);
    ...
    IEnumerator StandardBehaviour()
    {
        ...
        {
            //energyFill = Mathf.Lerp(energyFill, targetEnergyFill, Time.deltaTime * fillRate);  // lerp is bugged

            //Debug.Log("energyFill = Mathf.Lerp(energyFill, targetEnergyFill, Time.deltaTime * fillRate); " + energyFill + ", " + targetEnergyFill + ", " + Time.deltaTime * fillRate);
