...
[AddComponentMenu("UI/ScrollRectInfinite")]
public abstract class InfiniteScroll : ScrollRect
{
    ...
    private void Update()
    {
        Transform lastChild = content.GetChild(transform.childCount - 1);
        //Debug.Log("child  count = " + content.childCount);
        //Debug.Log("sibling count  = " + lastChild.GetSiblingIndex());
        UpdateContents();
    }
    ...
    void UpdateContents()
    {
        ...
        // GetDimension returns 'y' value
        if (GetDimension(content.sizeDelta) - (GetDimension(content.localPosition) * OneOrMinusOne()) < GetDimension(t.sizeDelta))
        {
            Debug.Log(GetDimension(content.sizeDelta) - (GetDimension(content.localPosition) * OneOrMinusOne()) + " < " + GetDimension(t.sizeDelta) + " = " + (GetDimension(content.sizeDelta) - (GetDimension(content.localPosition) * OneOrMinusOne()) < GetDimension(t.sizeDelta)));
            NewItemAtEnd();
