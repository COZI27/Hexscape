...
public class ScoreUI : MonoBehaviour
{
    ...
    void Start() {
        ...
        if (scoreIncreaseObj != null)
        {
            ...
            for (int i = 0; i < numberOfIncreaseObjects - 1; i++)
            {
                ...
                newObj.transform.SetParent( scoreIncreaseObj.transform.parent);
    ...
    public void SetScore(int totalScore, int currentTier, bool isLevel = false)
    {
        this.totalScoreText.text = totalScore.ToString("D8");
        if (isLevel)
        {
            this.currentTierText.text = "Lv" + currentTier.ToString();
        }
        else
        {
    ...
    IEnumerator PlayScoreIncrementEffect(GameObject obj, int score, float duration = 2.0f)
    {
        ...
        Vector3 targetPosiVal = resetPos + new Vector3(0,70,0);
